...
problem += (c_outlier * lpSum(sv_outlier) \
          ...
          + c_assign_suboptimal * lpSum(dv_assign_suboptimal))
...
d_assign_date = pd.concat([pd.Series(d_assign.sum(axis = 1), index = l_date, name = 'count'),
                           pd.Series(d_assign.apply(lambda row: str(row[row].index.to_list()[0]), axis = 1), index = l_date, name = 'dr')],
                           axis = 1)                      
d_assign_dr = pd.concat([pd.Series(d_assign.sum(axis = 0), index = l_dr, name = 'count'),
                         pd.Series(d_assign.apply(lambda col: col[col].index.to_list(), axis = 0), index = l_dr, name = 'date')],
                         axis = 1)
